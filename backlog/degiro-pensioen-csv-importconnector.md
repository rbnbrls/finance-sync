...
- [ ] Er is een ingebouwde connector met provider key `degiro_pension`,
...
- [ ] De connector gebruikt uitsluitend door de gebruiker aangeleverde
...
- [ ] De connector accepteert CSV en de actuele Excel-exportformaten voor
...
- [ ] Het rapporttype wordt veilig uit headers en structuur herkend. Bekende
...
- [ ] Het transactieoverzicht mapt aankopen en verkopen met datum/tijd, order-ID,
  ISIN, product, venue, quantity, prijs, instrumentvaluta, EUR-waarde,
  wisselkoers en transactiekosten naar canonical data.
- [ ] Het rekeningoverzicht mapt minimaal stortingen, opnames, dividend,
...
- [ ] Het portefeuilleoverzicht maakt een holdingsnapshot met ISIN, product,
  quantity, prijs, marktwaarde, cost basis/GAK wanneer aanwezig en valuta. Cash
  wordt apart van effecten verwerkt.
- [ ] De account wordt aangemaakt als `account_type=investment` en
  ...
  persoonlijk gegeven.
- [ ] `current_balance` representeert de totale pensioenrekeningwaarde op het
...
- [ ] Externe transactie-ID's zijn stabiele hashes van genormaliseerde
...
- [ ] Overlappende datumexports en het opnieuw importeren van dezelfde bestanden
  ...
  geregistreerd.
- [ ] Parsing is locale- en encodingbestendig voor onder meer BOM, komma- en
  puntdecimalen, quoted komma's in productnamen en winter-/zomertijd. Geld en
  quantity worden uitsluitend met `Decimal` verwerkt.
- [ ] Golden fixtures bevatten synthetische of aantoonbaar geanonimiseerde
...
- [ ] Documentatie bevat de exacte handmatige exportstappen in DEGIRO, de drie
