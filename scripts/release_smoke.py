...
if not BASE_URL:
    sys.exit("SMOKE_BASE_URL is required (e.g. https://<staging-app-uuid>.7rb.nl)")
...
def wait_for_health(path: str) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        if status == 200:
            ...
            return
        print(f"  … GET {path} -> {status} ({body if isinstance(body, str) else json.dumps(body)[:120]})")
...
def main() -> int:
    ...
    if not token:
        sys.exit(f"✗ login response has no access_token: {json.dumps(body)[:200]}")
    ...
    if status != 200:
        sys.exit(f"✗ GET /api/v1/transactions failed (HTTP {status}): {json.dumps(body)[:200]}")
    if not isinstance(body, dict) or "meta" not in body:
        sys.exit(f"✗ GET /api/v1/transactions missing meta envelope: {json.dumps(body)[:200]}")
    meta = body["meta"]
    expected_keys = {"as_of", "currency", "next_cursor", "freshness"}
    if not isinstance(meta, dict) or not expected_keys.issubset(meta):
        sys.exit(f"✗ GET /api/v1/transactions meta envelope wrong shape: {json.dumps(meta)[:200]}")
    print(f"  ✓ GET /api/v1/transactions -> 200 (meta keys: {sorted(expected_keys)})")
