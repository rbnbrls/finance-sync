...
def check_key_provider_status() -> dict[str, Any]:
    ...
    try:
        # Try to load key provider configuration
        config_path = "config/managed-key-provider.json"
        if os.path.exists(config_path):
            with open(config_path) as f:
                config = json.load(f)

        # For demonstration, we'll create a mock key status
        # In reality, this would interface with the actual key management system
...
def _check_key_version_downgrade(
    ...
    Versions are deliberately compared as integers only when both values are
    numeric. Provider-specific identifiers may be opaque strings, in which
    case no downgrade conclusion can be made.
    """
