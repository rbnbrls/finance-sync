...
| ms.4.f.4 | Wealthfolio exporter | **DONE** | `exporter/wealthfolio/` (client, config, exporter, transaction_mapper, models), API wiring in `api/v1/exporters.py`, `tests/test_wealthfolio_exporter.py`, `tests/exporter/test_wealthfolio_client.py`, `tests/exporter/test_wealthfolio_contract.py`, `tests/exporter/test_wealthfolio_live_contract.py` + recorded live fixtures `tests/exporter/fixtures/live/` (auth status, login failure, unauthenticated rejection, authenticated accounts/activities-search/holdings — recorded from the live instance 192.168.3.50:8080 on 2026-08-17; see PR #266). Live state verified end-to-end: accounts mapped (FINANCE_SYNC provider), 3 smoke activities imported with `ID: smoke-txn-*` dedup comments, zero duplicates across repeated runs (t_b56d009f, PR #248), UI matches API (t_991b5fb5, PR #251). |
...
| ID | Item | Status | Evidence |
|---|---|---|---|
| lb.1 | Use labels `area:api`, `area:connector`, `area:data`, `area:enrichment`, `area:exporter`, `area:ops`, `security`, `good-first-issue`, `blocked:provider`, `priority:P0/P1/P2` | **DONE** | All 12 roadmap labels created on `rbnbrls/finance-sync` via GitHub API (2026-08-14): `area:api`, `area:connector`, `area:data`, `area:enrichment`, `area:exporter`, `area:ops`, `security`, `good-first-issue`, `blocked:provider`, `priority:P0`, `priority:P1`, `priority:P2`; verified via `GET /repos/rbnbrls/finance-sync/labels` (30 labels total, all pre-existing labels untouched). → G-08 |
...
- **Status: IMPLEMENTED** (PR #214). Export tables migrated in
  ...
  + per-account error detail, run marked failed when any account
  fails); `GET /exporters/runs?status=error` lists dead-lettered runs
  with `error_message` + `exporter_type`; `POST
  /exporters/{type}/runs/{id}/retry` re-runs a failed run (409 on
  non-failed runs / exporter-type mismatch, 404 on unknown run/type or
  disabled exporter) without data loss. Also fixes two latent run-API
  bugs surfaced by the new tests: UUID path params now parse to UUIDs
  before ORM comparison, and run ids are stringified in responses
  (pydantic v2 does not coerce UUID→str, which 500'd the old runs
  endpoints on real data).
