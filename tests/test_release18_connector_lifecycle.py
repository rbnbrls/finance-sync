...
def test_registry_lifecycle_reports_capabilities_and_rollback_safely() -> None:
    diagnostics = evaluate(
        _lifecycle(), today=date(2026, 8, 22), fixture_version="2026-01-15"
    )
    assert {item["connector"] for item in diagnostics} == {
        "bunq",
        "trading212",
        "degiro_pension",
        "ynab",
...
def test_deprecation_feature_flag_and_fixture_failures_are_explicit() -> None:
    ...
    deprecated = evaluate(
        lifecycle, today=date(2026, 8, 22), fixture_version="2026-01-15"
    )
    ...
    old_fixture = evaluate(
        _lifecycle(), today=date(2026, 8, 22), fixture_version="2025-01-01"
    )
