...
def _baseline() -> dict[str, Any]:
    return {
        "metadata": {"postgres_version": "PostgreSQL 16"},
        "operations": {"portfolio": {"latency_ms": 10}},
    }
...
def test_query_budget_is_hard_and_latency_is_configurable() -> None:
    assert (
        compare(
            _baseline(), _current(), latency_tolerance=0.25, fail_latency=False
        )["failures"]
        == []
    )
    result = compare(
        _baseline(),
        _current(query_count=3),
