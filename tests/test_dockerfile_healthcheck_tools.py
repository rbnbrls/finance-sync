...
def _production_stage(path: Path) -> str:
    ...
    assert marker in dockerfile, f"production stage marker not found in {path.name}"
...
def test_production_stage_removes_unneeded_systemd_libraries() -> None:
    ...
    for path in (DOCKERFILE, DOCKERFILE_WORKER):
        stage = _production_stage(path)
        assert "apt-get purge" in stage, f"systemd cleanup missing from {path.name}"
        assert "libsystemd0" in stage, f"libsystemd0 cleanup missing from {path.name}"
...
def test_production_stage_has_healthcheck() -> None:
    ...
    for path in (DOCKERFILE, DOCKERFILE_WORKER):
        ...
        assert "/health/live" in stage, f"healthcheck target missing from {path.name}"
