...
logger = structlog.get_logger(
    "finance_sync.services.subscription_detector.detector"
)
...
def _detect_frequency_robust(  # pyright: ignore[reportUnusedFunction]
    ...
    First removes **outlier intervals** — intervals that are more
    than 2x the median (likely skipped payments) — then delegates to
    the standard ``_detect_frequency`` on the cleaned set.

    Example: intervals ``[30, 30, 60, 30, 32]`` → median 30,
    only ``60`` is an outlier (> 2 x 30) → removed → cleaned
    ``[30, 30, 30, 32]`` → monthly detected.

    Returns:
        Same shape as ``_detect_frequency``.
    """
    if not intervals_days:
        return None, None
    if len(intervals_days) < 2:
        return _detect_frequency(intervals_days)

    median_interval = median(intervals_days)
    if median_interval <= 0:
        return _detect_frequency(intervals_days)

    # Filter: keep intervals within 0.5x .. 2.0x of the median
    lower_bound = median_interval * 0.5
    upper_bound = median_interval * 2.0
    filtered = [i for i in intervals_days if lower_bound <= i <= upper_bound]

    if len(filtered) < 1:
        return _detect_frequency(intervals_days)

    return _detect_frequency(filtered)
...
def _amounts_are_consistent(
    ...
    Also detects **step-change patterns** where amounts shift between
    a small number of distinct levels over time (e.g. a price increase
    from €9.99 to €15.99).  When the overall variance exceeds 30 % the
    function falls back to cluster-based analysis — if the amounts
    naturally form a small number of tight clusters, a moderate
    consistency score (0.5) is returned rather than 0.0.
    """
    if len(amounts) < 2:
        return 1.0
    ...
    # ── Step-change fallback ───────────────────────────────────────────
    # When variance exceeds 30 %, check whether the amounts form a
    # small number of tight clusters (price changes over time).  If so,
    # return a moderate consistency score instead of 0.0.
    return _amounts_step_change_score(abs_amounts)


def _amounts_step_change_score(abs_amounts: list[Decimal]) -> float:
    """Score amounts that form distinct clusters (step changes).

    Clusters amounts within a 5 % relative + €2 absolute tolerance,
    then assesses the pattern:

    * 1 cluster of tight values — should have been caught above,
      so this is defensive (0.6).
    * 2-3 tight clusters — clear step-change pattern (0.5).
    * 4+ clusters or all noise — not a clean step change (0.0).
    ...
    # Build clusters using 1-D density grouping (DBSCAN-like)
    n = len(abs_amounts)
    neighbours: list[set[int]] = [set() for _ in range(n)]
    for i in range(n):
        for j in range(i + 1, n):
            a, b = abs_amounts[i], abs_amounts[j]
            diff = abs(a - b)
            mean_ab = (a + b) / Decimal(2)
            if diff <= _MAX_AMOUNT_ABSOLUTE or (
                mean_ab > Decimal(0)
                and diff / mean_ab <= _MAX_AMOUNT_VARIANCE_PCT
            ):
                neighbours[i].add(j)
                neighbours[j].add(i)

    # Core points need at least 2 neighbours (including self = min 2)
    # Using min_points=2 so even small clusters form
    core = {i for i in range(n) if len(neighbours[i]) + 1 >= 2}
    ...
    if num_clusters == 0:
        # No clusters formed — amounts are truly inconsistent
        return 0.0
    if num_clusters <= 1:
        # Single cluster — amounts are tight, should have been caught
        # by the relative-variance check above.  Defensive fallback.
        return 0.6
    if num_clusters <= 3:
        # 2-3 distinct amount levels — clear step-change pattern
        return 0.5
    # Too many clusters — amounts are truly inconsistent
    return 0.0
...
def _merge_cross_validated(
    ...
    """Merge a merchant-based result with a clustering result for the
    same merchant when both methods independently detected it.

    Cross-validation confirms the subscription, so:

    * A +0.10 cross-validation bonus is added to the detection score
      (capped at 1.0).
    * The detection method is upgraded to :attr:`DetectionMethod.HYBRID`.
    * The higher-scoring result is used as the base and enriched with
      details from the other.
    * Transaction IDs and occurrence counts are merged (deduplicated and
      maximised respectively).

    Returns:
        A new merged result dict.
    """
    merchant_score = merchant_result.get("detection_score", 0.0) or 0.0
    cluster_score = cluster_result.get("detection_score", 0.0) or 0.0

    # Start with the higher-scoring result as the base
    if merchant_score >= cluster_score:
        base = dict(merchant_result)  # shallow copy
    else:
        base = dict(cluster_result)

    # Apply cross-validation bonus
    cv_bonus = 0.10
    base_detection_score = base.get("detection_score", 0.0) or 0.0
    new_score = min(1.0, round(base_detection_score + cv_bonus, 4))
    base["detection_score"] = new_score

    # Upgrade confidence if threshold is crossed
    if new_score >= 0.80:
        base["confidence"] = SubscriptionConfidence.HIGH
    elif new_score >= 0.50 and (
        base.get("confidence") == SubscriptionConfidence.LOW
    ):
        base["confidence"] = SubscriptionConfidence.MEDIUM

    # Upgrade detection method
    base["detection_method"] = DetectionMethod.HYBRID

    # Merge details
    base_details = dict(base.get("details", {}) or {})
    base_details["cross_validated"] = True
    base_details["merchant_detection_score"] = merchant_score
    base_details["cluster_detection_score"] = cluster_score
    base_details["cross_validation_bonus"] = cv_bonus

    # Merge transaction IDs (deduplicated)
    all_ids: list[str] = list(base.get("transaction_ids", []) or [])
    cluster_ids: list[str] = list(
        cluster_result.get("transaction_ids", []) or []
    )
    for tid in cluster_ids:
        if tid not in all_ids:
            all_ids.append(tid)
    base["transaction_ids"] = all_ids

    # Merge occurrence_count (take max)
    merchant_occ = merchant_result.get("occurrence_count", 0) or 0
    cluster_occ = cluster_result.get("occurrence_count", 0) or 0
    base["occurrence_count"] = max(merchant_occ, cluster_occ)

    # Keep sector/security_id/category if available from either source
    for field in ("sector", "security_id", "category"):
        if (
            field in cluster_result
            and cluster_result[field]
            and not base.get(field)
        ):
            base[field] = cluster_result[field]

    base_details["merged_transaction_count"] = len(all_ids)
...
class SubscriptionDetector:
    ...
    async def _run_all_detection(
        ...
        *,
        ...
        Combines:
        1. Existing merchant-based grouping analysis with optional
           merchant classification.
        2. Clustering-based amount and period detection with cross-account
           matching.
        3. Cross-referencing — when the same merchant is detected by both
           methods independently, confidence is boosted and the detection
           method is upgraded to HYBRID.

        Cross-referenced results (both methods agree) are placed first,
        followed by unique findings from each individual method, then
        deduplicated by merchant name keeping the highest-quality entry.
        """
        ...
        # 3. Cross-reference: merchants detected by BOTH methods get a
        #    confidence boost and HYBRID detection method upgrade
        if merchant_results and cluster_results:
            results = self._cross_reference_results(
                merchant_results, cluster_results
            )
        else:
            results = list(merchant_results)
            results.extend(cluster_results)

        # 4. Final deduplication safety net
    ...
    def _cross_reference_results(
        ...
        """Cross-reference merchant-based and clustering-based results.

        When both methods independently detect the same merchant, it is
        strong evidence the transaction is genuinely a subscription.  For
        these overlapping merchants:

        * A cross-validation bonus (+0.10) is added to the detection score.
        * The detection method is upgraded to :attr:`DetectionMethod.HYBRID`.
        * Details from both sources are merged (transaction IDs, occurrence
          count, sector/security_id).
        * The richer result (higher base score) provides the foundation.

        Merchants detected by only one method are preserved as-is.
        Cross-referenced entries are placed first in the output since they
        are the most reliable findings.

        Args:
            merchant_results: Results from merchant-based grouping analysis.
            cluster_results: Results from clustering-based pattern detection.

        Returns:
            Combined list with cross-referenced entries merged and boosted.
        """
        if not merchant_results or not cluster_results:
            results = list(merchant_results)
            results.extend(cluster_results)
            return results

        # Build lookup maps (keep highest-score entry per merchant)
        cluster_map: dict[str, dict[str, Any]] = {}
        for r in cluster_results:
            merchant = r.get("merchant_name", "")
            if not merchant:
                continue
            existing = cluster_map.get(merchant)
            if existing is None or (r.get("detection_score", 0.0) or 0.0) > (
                existing.get("detection_score", 0.0) or 0.0
            ):
                cluster_map[merchant] = r
        ...
        for r in merchant_results:
            ...
            if not merchant:
                continue
            existing = merchant_map.get(merchant)
            if existing is None or (r.get("detection_score", 0.0) or 0.0) > (
                existing.get("detection_score", 0.0) or 0.0
            ):
                merchant_map[merchant] = r

        # Identify merchants detected by BOTH methods
        cross_merchants: set[str] = set(merchant_map.keys()) & set(
            cluster_map.keys()
        )
        ...
        # First pass: cross-referenced merchants (most reliable)
        for merchant in sorted(cross_merchants):
            merged = _merge_cross_validated(
                merchant_map[merchant], cluster_map[merchant]
            )
            results.append(merged)
            seen.add(merchant)

        # Second pass: merchants only detected by merchant grouping
        for r in merchant_results:
            m = r.get("merchant_name", "")
            if m and m not in seen:
                results.append(r)
                seen.add(m)

        # Third pass: merchants only detected by clustering
        for r in cluster_results:
            m = r.get("merchant_name", "")
            if m and m not in seen:
                results.append(r)
        ...
        self._log.info(
            ...
            total=len(results),
            cross_validated=len(cross_merchants),
        )
        return results
